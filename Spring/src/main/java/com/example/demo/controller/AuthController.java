...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<AuthResponse>> registerUser(@Valid @RequestBody RegisterRequest registerRequest) {
        ...
        Role userRole = roleRepository.findByName("USER")
            .orElseThrow(() -> new BusinessException("ROLE_NOT_FOUND", "無法找到使用者角色", HttpStatus.INTERNAL_SERVER_ERROR));
    ...
    public ResponseEntity<ApiResponse<AuthResponse>> login(@Valid @RequestBody LoginRequest loginRequest) {
        ...
        if (accountIdentifier == null || accountIdentifier.isEmpty()) {
            throw new BusinessException("INVALID_INPUT", "必須提供用戶名或電子郵件", HttpStatus.BAD_REQUEST);
        }

        User user = (loginRequest.getUsername() != null && !loginRequest.getUsername().isEmpty()
                ? userRepository.findByUsername(loginRequest.getUsername())
                : userRepository.findByEmail(loginRequest.getEmail()))
            .orElseThrow(() -> new BusinessException("USER_NOT_FOUND", "用戶不存在", HttpStatus.NOT_FOUND));

        if (!user.isEnabled()) {
            throw new BusinessException("ACCOUNT_DISABLED", "帳戶已被禁用", HttpStatus.FORBIDDEN);
        }
    ...
    public ResponseEntity<ApiResponse<AuthResponse>> refreshToken(@AuthenticationPrincipal User currentUser) {
        ...
        jwtIdService.findByJti(jti)
            .orElseThrow(() -> new BusinessException("REVOKED_TOKEN", "Refresh Token 不存在或已被註銷", HttpStatus.UNAUTHORIZED));
